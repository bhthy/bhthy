...
$app->GET('/api/v1/users/{userId}', function ($request, $response, $args) {
    ...
    if (!empty($user)) {
		$user->makeVisible(['user_login_count']);
		$result['data'] = $user;
...
$app->GET('/api/v1/me', function ($request, $response, $args) {
	...
	if(isset($authUser) && isset($authUser->id) && $authUser->id != "") {
    ...
    try {
        ...
        if(!empty($data)) {
			...
			foreach($data as $datas) {
				$data[$i]["is_active"] = ($datas["is_active"] == "1")?true:false; 
				$data[$i]["is_test_mode"] = ($datas["is_test_mode"] == "1")?false:true;
				$i++;
			}
		}
		$result = array(
...
$app->GET('/api/v1/payment_gateways/{paymentGatewayId}', function ($request, $response, $args) {
    ...
    if (!empty($paymentGateway)) {
		...
		if ($result['data']['is_test_mode'] == "0") {
			$result['data']['is_test_mode'] = true;
		} else {
			$result['data']['is_test_mode'] = false;
		}
        return renderWithJson($result);
    } else {
        return renderWithJson($result, 'No record found', '', 1);
...
$app->PUT('/api/v1/payment_gateway_settings/{paymentGatewayId}', function ($request, $response, $args) {
	...
	if ($request->getAttribute('paymentGatewayId') == "3") {
		if ($is_live == 1) {
			Models\PaymentGatewaySetting::where('name', 'paypal_client_id')->update(array(
				'live_mode_value' => $body['live_mode_value']['paypal_client_id']
			));
			Models\PaymentGatewaySetting::where('name', 'paypal_client_Secret')->update(array(
					'live_mode_value' => $body['live_mode_value']['paypal_client_Secret']
				));
		} else {
			Models\PaymentGatewaySetting::where('name', 'paypal_client_id')->update(array(
					'test_mode_value' => $body['test_mode_value']['paypal_client_id']
				));
			Models\PaymentGatewaySetting::where('name', 'paypal_client_Secret')->update(array(
					'test_mode_value' => $body['test_mode_value']['paypal_client_Secret']
				));
		}
		Models\PaymentGateway::where('name', 'PayPalREST')->update(array(
			 "is_test_mode" => $is_live
		));
	}
...
$app->POST('/api/v1/attachments', function ($request, $response, $args) {
    ...
    if(!empty($file)) {
		...
		$currentFileName = str_replace('.'.$type,"",$newfile->getClientFilename()); 
        $fileName = $currentFileName.'_'.time().'.'.$type;
		$name = md5(time());
        $class = $args['class'];
            ...
            if (empty($max_file_size) || (!empty($max_file_size) && $current_file_size <= $max_file_size)) {
                if ($class == "UsersMyFile") {
                    ...
                    if (move_uploaded_file($newfile->file, $filePath.$fileName) === true) {
						...
						$extType = explode(".",$fileNameArr[1]);
						$attachment->filename = $fileNameArr[0].$extType[1];
						$attachment->description = $fileNameArr[1];
                    ...
                    if (move_uploaded_file($newfile->file, APP_PATH . '/media/tmp/'. $name.'_'.$currentFileName.'.' . $type) === true) {
                        $filename = $name.'_'.$currentFileName.'.' . $type;
                        $response = array(
                            'attachment' => $filename,
                            'error' => array(
                                'code' => 0,
 ...
 * Output-Formats: [application/json]
 */
$app->GET('/api/v1/quote_cron', function ($request, $response, $args) {
	global $_server_domain_url;
	global $capsule;
    ...
    if (!empty($projects)) {
        foreach ($projects as $project) {
			$projectSkills = Models\SkillsProjects::select('skill_id')->where('project_id', $project['id'])->get()->toArray();
			$skillIds = array();
			foreach ($projectSkills as $skillId) {
				$skillIds[] = $skillId['skill_id'];
			}
			if(!empty($skillIds)) {
				$user_skills = Models\SkillsUser::select('user_id')->distinct()->whereIn('skill_id', $skillIds)->get()->toArray();
				if(!empty($user_skills)) {
					$usersIds = array();
					foreach ($user_skills as $user_skill) {
						$userDetails = getUserHiddenFields($user_skill['user_id']);
						$emailFindReplace = array(
						...
						sendMail('Project Posted Alert', $emailFindReplace, $userDetails->email);
					}
            ...
            switch ($project['project_status_id']) {
                case \Constants\ProjectStatus::OpenForBidding:
                    ...
                    if (strtotime($project['project_bid']['bidding_end_date']) <= strtotime(date('Y-m-d 23:59:59'))) {
                        ...
                        $project_status_count = Models\Project::where('project_status_id', $project_status_id)->count();
						Models\ProjectStatus::where('id', $project_status_id)->update(['project_count' => $project_status_count]);
	...
	$skillsUser = Models\SkillsUser::select('skill_id', $capsule::raw('COUNT(skill_id) as count'))->groupBy("skill_id")->havingRaw("COUNT(skill_id) > 1")->get();
	$skillsProjects = Models\SkillsProjects::select('skill_id', $capsule::raw('COUNT(skill_id) as count'))->groupBy("skill_id")->havingRaw("COUNT(skill_id) > 1")->get();
	if(!empty($skillsUser)) {
		foreach ($skillsUser as $skillUser){
			Models\Skill::where('id', $skillUser->skill_id)->increment('user_count', (int)$skillUser->count);
		}
	}
	if(!empty($skillsProjects)) {
		foreach ($skillsProjects as $skillsProject){
			Models\Skill::where('id', $skillsProject->skill_id)->increment('project_count', (int)$skillsProject->count);
		}
	}
	/*$result = array();
    ...
    if (!empty($langCode)) {
        ...
        if (file_exists($lang_file_path)) {
            ...
            if (!empty($args['keyword'])) {
				foreach ($args['keyword'] as $key => $value) {
                    $app[$key] = $value;
                }
            }
            $fh = fopen($lang_file_path, 'w');
            fwrite($fh, json_encode($app,JSON_UNESCAPED_SLASHES));
            fclose($fh);
            $result['data'] = $app;
            return renderWithJson($app);
        } else {
